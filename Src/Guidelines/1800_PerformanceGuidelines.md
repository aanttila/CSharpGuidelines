...
### <a name="av1800"></a> Always use `Any()` to determine whether an `IEnumerable<T>` is empty (AV1800) ![](images/3.png)
...
### <a name="av1810"></a> Always use classes from `System.Collections.Generics` instead of `System.Collections` (AV1810) ![](images/1.png)
The classes in `System.Collections.Generics` avoid the boxing/unboxing operations required by classes in `System.Collections`.  Using generics also enforces type safety when working with collections.

### <a name="av1820"></a> Prefer `async` for low-intensive long-running activities (AV1820) ![](images/1.png)
...
### <a name="av1830"></a> Avoid mixing `await`/`async` with `Task.Wait` (AV1830) ![](images/1.png)
...
### <a name="av1835"></a> Avoid `async`/`await` deadlocks in single-threaded environments (AV1835) ![](images/1.png)
...
### <a name="av1840"></a> Always wrap `IDisposable` instances in a `using` statement (AV1840) ![](images/1.png)
Any class that implements `IDisposable` should have its `Dispose` method called as soon as it is no longer needed, preferably with a `using` statement, to avoid memory leaks:
